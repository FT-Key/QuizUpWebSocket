...
async function getDashboardState(): Promise<GameType[]> {
  const gamesDocs = await GameModel.find()
    .sort({ createdAt: -1 })
    .lean<GameDoc[]>();
  return gamesDocs.map((g: GameDoc) => ({
    id: g._id.toString(),
    name: g.name,
    status: g.status as GameType["status"],
...
async function emitGameUpdate(gameId: string) {
  const gameDoc = await GameModel.findById(gameId).lean<GameDoc>();
  if (!gameDoc) return;
  const game: GameType = {
...
io.on("connection", (socket) => {
  ...
  // ---- Start game
  socket.on("start-game", async ({ gameId }: { gameId: string }) => {
  ...
  // ---- Submit answer
  socket.on("submit-answer", async ({ gameId }: { gameId: string }) => {
    const games = await getDashboardState();
    io.emit("update-dashboard", games);
    await emitGameUpdate(gameId);
  });
  ...
  // ---- Finish game
  socket.on("finish-game", async ({ gameId }: { gameId: string }) => {
    const games = await getDashboardState();
    io.emit("update-dashboard", games);
    await emitGameUpdate(gameId);
