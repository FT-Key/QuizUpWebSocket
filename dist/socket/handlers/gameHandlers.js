...
import { emitDashboard, buildGame } from "../helpers.js";
...
function startQuestionTimeout(io, gameId, timeLimit) {
    ...
    const timeout = setTimeout(async () => {
        ...
        try {
            const bulkOps = game.players.map((p) => {
                const answersObj = {};
...
function clearQuestionTimeout(gameId) {
    if (activeQuestionTimeouts.has(gameId)) {
        clearTimeout(activeQuestionTimeouts.get(gameId));
        activeQuestionTimeouts.delete(gameId);
    }
}
const calcTimeLeftMs = (startTime, timeLimit) => Math.max(0, timeLimit - (Date.now() - startTime));
export default function registerGameHandlers(io, socket) {
    ...
    socket.on("finish-game", async ({ gameId }) => {
        ...
        try {
            if (gameBefore) {
                const bulkOps = gameBefore.players.map((p) => {
                    const answersObj = {};
                    if (p.answers instanceof Map) {
                        for (const [k, v] of p.answers)
                            answersObj[k] = v;
    ...
    socket.on("request-game-state", async ({ gameId }) => {
        ...
        if (game.status === "waiting" && isWaitingGameExpired(game.createdAt)) {
            try {
                ...
                console.error(`[gameHandlers] no se pudo cancelar la partida expirada ${gameId}:`, err);
            }
