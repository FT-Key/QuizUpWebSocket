...
export async function onLeaveGame(io, socket, { gameId, playerId }) {
    await GameModel.findOneAndUpdate({ gameCode: gameId }, { $pull: { players: { id: playerId } } });
...
export default async function onJoinGame(io, socket, { gameId, playerId, playerName, avatar }) {
    ...
    if (!playerId && playerName) {
        ...
        if (gameDoc.status === "waiting" &&
            ...
            try {
                ...
                console.error(`[playerHandlers] no se pudo cancelar la partida expirada ${gameId}:`, err);
            }
        ...
        const dbPlayer = gameDoc.players.find((p) => p.id === playerId);
        if (dbPlayer) {
            const plainAnswers = {};
            if (dbPlayer.answers instanceof Map) {
                for (const [k, v] of dbPlayer.answers)
                    plainAnswers[k] = v;
            }
    ...
    const playersInDb = gameDoc.players;
    ...
    if (!storeGame) {
        const questions = (gameDoc.questions || []).map((q) => ({
            id: q._id?.toString() || "",
            text: q.text,
            options: q.options,
        ...
        const plainPlayers = playersInDb.map((p) => {
            const plainAnswers = {};
            if (p.answers instanceof Map) {
                for (const [k, v] of p.answers)
                    plainAnswers[k] = v;
            }
    ...
    const currentQuestion = storeGame.questions[storeGame.currentQuestionIndex] || null;
    io.to(`game-${gameId}-admins`).emit("game-state", {
        game: storeGame,
        currentQuestion,
        currentQuestionIndex: storeGame.currentQuestionIndex,
        timeLeft: storeGame.currentQuestionStartTime > 0
